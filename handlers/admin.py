...
from services import analytics, backup, broadcast, pdf_report, report
...
async def outliers(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    for row, total, median in suspects[:25]:
        lines.append(
            f"{row.get('name')} / roll {row.get('roll_no')} - {fmt(total)} "
...
async def send_pdf(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await context.bot.send_document(
        ...
        filename=name,
        caption="Share this on WhatsApp groups, it travels further than a Telegram post.",
    )
