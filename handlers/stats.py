...
async def cmd_delete(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Ask first. This is irreversible and easy to fire by accident."""
    candidate = await db.get_candidate(update.effective_user.id)
    if not candidate:
        await update.effective_message.reply_text(texts.DELETE_NOTHING)
        return
    await update.effective_message.reply_text(
        texts.DELETE_CONFIRM, reply_markup=kb.confirm_delete()
    )


async def do_delete(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    outcome = await db.anonymise_candidate(update.effective_user.id)
    if outcome == "not_found":
        await update.effective_message.edit_text(texts.DELETE_NOTHING)
        return
    await update.effective_message.edit_text(
        texts.DELETED if outcome == "anonymised" else texts.DELETED_FULL
    )
