...
async def _next_ghost_id() -> int:
    """A guaranteed-free negative id. Real Telegram ids are always positive."""
    row = await _fetchone("SELECT MIN(telegram_id) AS m FROM candidates")
    lowest = row["m"] if row and row["m"] is not None else 0
    return min(lowest, 0) - 1


async def anonymise_candidate(telegram_id: int) -> str:
    """Erase who they are, keep what they scored.

    Name, roll number and the Telegram link are destroyed. The score row
    survives under a synthetic negative id, so submission counts and medians
    are unaffected and nothing traces back to the person - not even from the
    admin panel.

    Returns 'anonymised', 'deleted' or 'not_found'.
    """
    row = await get_candidate(telegram_id)
    if not row:
        return "not_found"

    # Someone who asked to be forgotten must not receive a queued broadcast.
    await _exec("DELETE FROM sessions WHERE telegram_id = ?", (telegram_id,))
    await _exec(
        "DELETE FROM broadcast_queue WHERE telegram_id = ? AND status = 'pending'",
        (telegram_id,),
    )
    await _exec(
        "UPDATE candidates SET referred_by = NULL WHERE referred_by = ?",
        (telegram_id,),
    )

    if row.get("status") != "submitted" or row.get("total_marks") is None:
        # Registered but never submitted: no score worth keeping, so the row
        # goes entirely rather than lingering as an empty ghost.
        await _exec(
            "DELETE FROM submission_history WHERE telegram_id = ?", (telegram_id,)
        )
        await _exec("DELETE FROM candidates WHERE telegram_id = ?", (telegram_id,))
        return "deleted"

    ghost = await _next_ghost_id()
    await _exec(
        """
        UPDATE candidates
           SET telegram_id = ?, name = NULL, roll_no = NULL,
               referred_by = NULL, referral_count = 0, updated_at = ?
         WHERE telegram_id = ?
        """,
        (ghost, _now(), telegram_id),
    )
    await _exec(
        "UPDATE submission_history SET telegram_id = ? WHERE telegram_id = ?",
        (ghost, telegram_id),
    )
    return "anonymised"


async def anonymised_count() -> int:
    row = await _fetchone(
        "SELECT COUNT(*) AS n FROM candidates WHERE telegram_id < 0"
    )
