...
def admin_home() -> M:
    return M(
        [
            ...
            [B("Normalisation Estimate", callback_data="admin:norm")],
            [B("Answer Key Coverage", callback_data="admin:keys")],
            [B("Outlier Queue", callback_data="admin:outliers")],
            [
                B("Public PDF", callback_data="admin:pdf"),
                B("Detailed PDF", callback_data="admin:pdf_full"),
            ],
...
def confirm_delete() -> M:
    return M(
        [
            [B("Yes, erase my identity", callback_data="data:delete_yes")],
            [B("Cancel", callback_data="menu:cancel")],
        ]
    )
